"""
fit_model_xbh.py — Fit Model 3 logistic regression for XBH (doubles + triples + HRs).

Stub: mirrors fit_model.py structure but labels PAs as XBH instead of HR.
Output goes to models_out/fitted_coefficients_xbh.json.
"""
...
def resolve(df: pd.DataFrame, spec: dict, label: str) -> dict:
    ...
    print(f"[fit_xbh] {label} column mapping resolved {len(resolved)}/{len(spec)}")
...
def main():
    ...
    print(f"[fit_xbh] join success: batter={bat_join_pct:.1%}  pitcher={pit_join_pct:.1%}")

    feature_cols = list(bat_map.keys()) + list(pit_map.keys())
    complete = df.dropna(subset=feature_cols).copy()
    print(f"[fit_xbh] complete-case PAs: {len(complete):,} / {len(df):,} "
          f"({len(complete)/len(df):.1%}); XBH rate in fit set: {complete['xbh'].mean():.4f}")
    ...
    y = complete["xbh"].values.astype(int)
    ...
    metrics = {
        "n": int(len(y)),
        "xbh_rate": float(y.mean()),
        "log_loss": float(log_loss(y, p)),
        "brier": float(brier_score_loss(y, p)),
        "auc": float(roc_auc_score(y, p)),
    }
    print(f"[fit_xbh] in-sample metrics: {metrics}")
    ...
    print("[fit_xbh] calibration (predicted vs actual):")
    ...
    try:
        ...
        ax.set_xlabel("Predicted XBH probability")
        ax.set_ylabel("Actual XBH rate")
        ax.set_title(f"XBH Calibration ({season}, n={metrics['n']:,})")
        plot_path = MODELS_DIR / "calibration_plot_xbh.png"
        fig.savefig(plot_path, dpi=120, bbox_inches="tight")
        print(f"[fit_xbh] wrote {plot_path}")
    except Exception as e:
        print(f"[fit_xbh] plot skipped: {e}")
    ...
    out_path = MODELS_DIR / "fitted_coefficients_xbh.json"
    out_path.write_text(json.dumps(out, indent=2))
    print(f"[fit_xbh] wrote {out_path}")
