...
Stub: mirrors models.py structure but with baseline calibrated to H/PA ≈ 0.245.
...
# HIT BASELINE
# --------------------------------------------------------------------------
HIT_PER_PA = 0.245  # league hits per PA baseline
...
M1_WEIGHTS = dict(barrel=0.28, xslg=0.20, hardhit=0.16, ev=0.12, k=-0.10, la=0.08, whiff=-0.06)  # TODO: fit for hits
...
def model2_matchup(name):
    ...
    dmg_rise = barrel_z + 0.5 * la_z - 0.5 * whiff_z      # TODO: fit for hits
    dmg_sink = barrel_z - 0.4 * topped_z                    # TODO: fit for hits
    dmg_soft = -0.6 * chase_z - 0.6 * whiff_z + 0.3 * barrel_z  # TODO: fit for hits
...
M3 = dict(
    intercept=math.log(HIT_PER_PA / (1 - HIT_PER_PA)),  # baseline log-odds for hits
    barrel=0.55, xslg=0.35, hardhit=0.20, la=0.15, whiff=-0.20, matchup=0.40,  # TODO: fit for hits
)
...
ENSEMBLE_W = dict(logistic=0.45, matchup=0.30, linear=0.25)  # TODO: fit for hits

CALIBRATION_DAMP = 0.45  # TODO: calibrate for hits
...
def _score_to_prob(scores, this_score):
    """Map a population of raw scores to probabilities anchored at hit baseline."""
    mu, sd = mean(scores), (pstdev(scores) or 1.0)
    base_logodds = math.log(HIT_PER_PA / (1 - HIT_PER_PA))
